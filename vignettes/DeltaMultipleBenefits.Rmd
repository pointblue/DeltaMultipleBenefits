...
__classify_landcover:__ The first step is again to create the predictor groupings required by the "riparian" models, but here it is best to work directly with the original LSPT Habitat_types_modern layer, if possible. The `classify_landcover.sf()` function applied to the LSPT data will return the corresponding predictors for the "riparian" models with the greatest accuracy and specificity. As an example:
...
Otherwise, `classify_landcover.SpatRaster()` can be used, but may produce some minor differences from the original land cover classification scheme used in the development of these models. 

__python_focal_prep:__ Following the classification step, the next step is again to prep for running focal statistics, as above. 

```{r riparian2, eval = TRUE}
