...
Next, draw on the finalized predictors created in the last step to fit the distribution models for each of the 9 riparian landbird species. As above, note that `pathin` in this function should be the same as `pathout` in the previous step, and that `landscape_name` should be identical. The `modlist` should refer to an R object containing a list of the riparian landbird models, downloaded via Zenodo, doi:[10.5281/zenodo.7531945](https://doi.org/10.5281/zenodo.7531945) and loaded into the R environment. Because the original models included `area.ha`, referring to the survey effort, and `region`, referring to whether the survey was conducted in the Sacramento Valley (region = 0) or in the Delta or San Joaquin (region = 1), these constant values must be passed in as a dataframe. In addition, we considered open water to be unsuitable land cover *a priori*, and so we specified `unsuitable = 90` (the land cover class value for open water), which will create a mask from the provided `landscape` with 0 values wherever the land cover class equals the values provided in `unsuitable` and NA elsewhere, used as a mask to replace the predicted values from the model. 

Thus, the original call to this function in our analyses for the Delta looked like this:
```{r riparian4, eval = FALSE}

DeltaMultipleBenefits::fit_SDM(pathin = 'SDM_predictors/predictors_riparian',
                               landscape_name = 'scenario_example',
                               modlist = BRT_riparian,
                               constants = data.frame(region = 1,
                                                      area.ha = 3.141593),
                               unsuitable = 90, #open water
                               landscape = rasters$scenario,
                               pathout = 'SDM_results/riparian',
                               overwrite = TRUE)
```

However, this code will not work correctly on our example scenario for two reasons: (1) the unsuitable land cover class value of 90 does not exist in our example scenario, and so will result in a mask landscape of all NA values, throwing an error, and (2) our example scenario does not include ALL of the predictors required in the original distribution model, including all land cover classes, climate variables, and distance to stream. To predict riparian landbird distributions for our example landscape, we need to include all required predictors, either as additional raster files in the `pathin` directory, or as constant values. For this example, we will treat the missing landcover classes as truly absent from the landscape, with a value of 0 for every pixel, and we will also assume the climate and distance to stream does not vary across our example landscape. We will also refrain from specifying any of these land covers as unsuitable *a priori*. 
...
*Note 2: In this example, we have included only one distribution model in `modlist` for speed, but the function is designed to accept and predict distributions for all 9 models at once.*
...
In addition, because the riparian landbird species varied in their prevalence across the landscape and in the precision of the distribution models, different thresholds in their predicted probability of presence are useful for separating locations where they are most likely to be present or absent. Therefore, to estimate the total area of suitable habitat for each species, and for riparian landbirds collectively, it is useful to convert the continuous probabilities of species presence predicted in the previous step to binary predictions of presence or absence. Again, this function will write new rasters to a new directory; `pathin` in this function should be the same as `pathout` in the previous step, and `SDM` and `landscape_name` should be identical. We still need the original list of models to access embedded detection data from which thresholds can be calculated. Here, as in our original analysis, we use the statistic `equal_sens_spec` to identify the threshold value for the predicted probability of presence at which specificity (the probability of correctly predicting species absence) and sensitivity (the probability of correctly predicting species presence) are equal, but other statistics can be used as in `dismo::threshold`.
...
First estimate the total habitat predicted for each species distribution model using the built-in function `sum_habitat`. By default, the function will recursively find all prediction rasters in the `pathin` argument, which should reflect the output of the previous step. Thus, if predictions have been made for riparian landbirds for multiple scenarios, or if `pathin` already contains predictions for waterbirds, this function only needs to be run once to summarize the total habitat predicted from each model. However, `SDM` or both `SDM` and `landscape_name` can optionally be provided to limit the predictions evaluated. 
...
Thus far, we have only been demonstrating the application of the riparian species distribution models to the example scenario, but to estimate the net change in habitat from the baseline landscape, all of these same steps would be repeated for the baseline landscape, and the habitat totals calculated in the previous step would include estimates under baseline conditions. The change in total habitat between each scenario and the baseline can then be readily calculated. Here, we will simulate baseline estimates and demonstrate calculating the net change. 

*Note that these habitat totals can be appended to the `scores` data developed in Step 3 above, reflecting the total landscape scores for each of the non-spatial metrics, and the same `sum_change` function can be used to simultaneously estimate the net change for non-spatial and spatial metrics.*

```{r riparian8, eval = FALSE}
habitat_baseline = tibble(
  scenario = 'baseline',
  METRIC_CATEGORY = 'Biodiversity Support',
  METRIC_SUBTYPE = 'Riparian landbird',
  METRIC = c('Ash-throated Flycatcher', 'Total'),
  SCORE_TOTAL = c(42, 42),
  UNIT = 'ha')

scores_change_habitat = DeltaMultipleBenefits::sum_change(
  bind_rows(habitat_totals, habitat_baseline))
```
