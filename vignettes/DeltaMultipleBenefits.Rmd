...
__python_focal_stats:__ The third step is to generate focal statistics for each of the separate land cover rasters generated in the previous step. Focal statistics perform a given operation on all cells within a given distance of the focal cell, and repeated for every cell in the raster. For the "tima" models, this function will calculate the mean value for the presence of every land cover class within each landscape to be analyzed, within moving windows on each of two spatial scales (50m and 2km). All input layers are read from `pathin/SDM/landscape_name`, processed, and then written out to `pathout/SDM/landscape_name/scale`. 
...
update_pwater(waterdat = pwater_base_fall, SDM = 'waterbird_fall',
              landscape_name = 'scenario',
              pathout = 'SDM_predictors', overwrite = TRUE,
