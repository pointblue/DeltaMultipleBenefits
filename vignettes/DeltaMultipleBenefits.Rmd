...
new_shp <- olinda %>% 
  dplyr::mutate(
    CODE_NAME = dplyr::case_when(
      ...
      TRUE ~ 'UNKNOWN'))
new_shp %>% dplyr::filter(CODE_NAME == 'UNKNOWN') # should be none!
```
...
# assign color coding
terra::coltab(z) <- key %>% select(CODE_BASELINE, COLOR) %>%
...
# assign color coding
terra::coltab(new_rast) <- key %>% select(CODE_BASELINE, COLOR) %>%
  tidyr::drop_na() %>% tidyr::complete(CODE_BASELINE = c(0:255)) %>% 
  dplyr::pull(COLOR)
terra::plot(new_rast)
```
